...
def customer_add(name: str = Form(...), email: str = Form(""), address: str = Form(""),
    ...
    con.commit()
    return safe_redirect("/customers")
...
def customer_update(customer_id: int = Form(...), name: str = Form(...), email: str = Form(""),
    ...
    con.commit()
    return safe_redirect(f"/customers/{customer_id}")

@router.get("/customers", response_class=HTMLResponse)
def customers_page(request: Request, con=Depends(get_con)):
...
def customer_detail(customer_id: int, request: Request, con=Depends(get_con)):
    err = request.query_params.get("err", "")
    msg = request.query_params.get("msg", "")

    customer = con.execute("SELECT * FROM customers WHERE id=?", (customer_id,)).fetchone()
    if not customer:
        return safe_redirect("/customers", err="Customer not found")
...
async def customer_upload_file(customer_id: int, file: UploadFile = File(...), kind: str = Form("tax_form"),
    ...
    return safe_redirect(f"/customers/{customer_id}", msg="File uploaded successfully")

@router.get("/customers/file/{file_id}")
def customer_download_file(file_id: int, con=Depends(get_con)):
    row = con.execute("SELECT * FROM customer_files WHERE id=?", (file_id,)).fetchone()
    if not row:
        return safe_redirect("/customers", err="File not found")
...
def customer_delete_file(file_id: int, con=Depends(get_con)):
    row = con.execute("SELECT * FROM customer_files WHERE id=?", (file_id,)).fetchone()
    if not row:
        return safe_redirect("/customers", err="File not found")
    ...
    return safe_redirect(f"/customers/{row['customer_id']}", msg="File deleted")

@router.post("/customers/{customer_id}/add-note")
def customer_add_note(customer_id: int, note: str = Form(...), con=Depends(get_con)):
    if not note.strip():
        return safe_redirect(f"/customers/{customer_id}", err="Note cannot be empty")

    con.execute("INSERT INTO customer_notes(customer_id, note) VALUES(?,?)", (customer_id, note.strip()))
    con.commit()
    return safe_redirect(f"/customers/{customer_id}", msg="Note added")

@router.post("/customers/note/{note_id}/delete")
def customer_delete_note(note_id: int, con=Depends(get_con)):
    row = con.execute("SELECT * FROM customer_notes WHERE id=?", (note_id,)).fetchone()
    if not row:
        return safe_redirect("/customers", err="Note not found")

    con.execute("DELETE FROM customer_notes WHERE id=?", (note_id,))
    con.commit()
    return safe_redirect(f"/customers/{row['customer_id']}", msg="Note deleted")

@router.get("/customers/{customer_id}/report", response_class=HTMLResponse)
def customer_report(customer_id: int, request: Request, con=Depends(get_con)):
    customer = con.execute("SELECT * FROM customers WHERE id=?", (customer_id,)).fetchone()
    if not customer:
        return safe_redirect("/customers", err="Customer not found")
