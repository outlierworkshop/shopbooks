...
from webutil import _INLINE_MEDIA, ctx, get_con, safe_redirect, templates

router = APIRouter()

@router.get("/receipts", response_class=HTMLResponse)
def receipts(request: Request, msg: str = "", err: str = "", con=Depends(get_con)):
...
def receipts_missing(request: Request, period: str = "this-year", minamt: str = "", con=Depends(get_con)):
    try:
...
async def receipts_upload(files: list[UploadFile] = File(...), con=Depends(get_con)):
    for f in files:
...
async def receipts_import_amazon(file: UploadFile = File(...), con=Depends(get_con)):
    try:
        orders = importer.parse_amazon_orders(await file.read())
    except ValueError as e:
        return safe_redirect("/receipts", err=str(e))
    ...
    return safe_redirect("/receipts", msg=note)

@router.post("/receipts/import-folder")
def receipts_import_folder(folder: str = Form(...), recursive: str = Form(""), con=Depends(get_con)):
    p = Path(folder.strip().strip('"'))
    if not p.is_dir():
        return safe_redirect("/receipts", err=f"Folder not found: {folder}")
    ...
    con.commit()
    if scanned == 0:
        return safe_redirect("/receipts", err=f"No receipt images found in {folder} (looked for jpg/png/gif/webp/pdf).")
...
def receipts_rematch(con=Depends(get_con)):
    matched = 0
    ...
    con.commit()
    return safe_redirect("/receipts", msg=f"Re-checked matches: {matched} newly matched.")

@router.post("/receipts/recategorize")
def receipts_recategorize(doc_id: int = Form(...), con=Depends(get_con)):
    ...
    return safe_redirect("/receipts", msg=msg if changed else None, err=None if changed else msg)

@router.post("/receipts/recategorize-all")
def receipts_recategorize_all(con=Depends(get_con)):
    ...
    if err and not changed:
        return safe_redirect("/receipts", err=err)
    return safe_redirect("/receipts", msg=
        f"Recategorized {changed} transaction(s) from their receipts. Review them in the registers; "
        "use the dropdown here to fix any you disagree with.")

@router.post("/receipts/setcategory")
def receipts_setcategory(doc_id: int = Form(...), account_id: int = Form(...), con=Depends(get_con)):
    ...
    if doc and doc["entry_id"]:
        ...
        con.commit()
        if old is None:
            return safe_redirect("/receipts", err="Couldn't change that one (not a simple expense, or different account type).")
    return RedirectResponse("/receipts", status_code=303)

@router.post("/receipts/match")
def receipts_match(doc_id: int = Form(...), entry_id: int = Form(...), con=Depends(get_con)):
...
def receipts_unmatch(doc_id: int = Form(...), con=Depends(get_con)):
...
async def save_staged_matches(request: Request, con=Depends(get_con)):
...
async def save_entry_matches(request: Request, con=Depends(get_con)):
    form = await request.form()
    doc_id = int(form["doc_id"])
...
def receipts_delete(doc_id: int = Form(...), con=Depends(get_con)):
...
def doc_file(doc_id: int, con=Depends(get_con)):
