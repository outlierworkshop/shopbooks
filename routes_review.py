...
async def do_import(request: Request, file: UploadFile = File(...), account_id: int = Form(None), con=Depends(get_con)):
    try:
        ...
        if account_id is not None:
            ...
            return safe_redirect("/review", msg=note)
...
def review(request: Request, note: str = "", err: str = "", con=Depends(get_con)):
    rows = con.execute(
...
@router.post("/review")
async def review_action(request: Request, con=Depends(get_con)):
    ...
    if "save_matches" in form:
        ...
        if is_split(sid):
            if not _post_staged(con, sid, None, splits=splits_for(sid)):
                con.commit()
                return safe_redirect("/review", err="The split amounts must add up to the transaction total. Nothing was posted.")
        else:
